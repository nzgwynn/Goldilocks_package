...
#' @param Min a numeric vector that is the minimum x value from
#'  brushing on the plot
#' @param Max a numeric vector that is the maximum x value from
#'  brushing on the plot
#'
...
make.zoom.plot <- function(data, I, Min, Max){
  ...
  p <- GGally::ggparcoord(data, columns = 1:(Lims[2] - Lims[1] + 1),
                          groupColumn = dim(data)[2],
                          scale = "globalminmax",
                          shadeBox = NULL) +
